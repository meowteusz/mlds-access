...
if [ $? -eq 0 ]; then
    ...
    HOST_INFO=$(uname -a)
    LOG_COMMAND="ssh -i $KEY_PATH $NETID@irc.mlds.northwestern.edu 'mkdir -p /nfs/home/shared/setup_logs && echo \"[$TIMESTAMP] Setup successful from $HOST_INFO\" >> /nfs/home/shared/migration/${NETID}_$(date +%Y%m%d%H%M%S).log'"
